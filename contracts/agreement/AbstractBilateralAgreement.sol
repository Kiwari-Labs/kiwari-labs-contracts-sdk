...
abstract contract AbstractBilateralAgreementProxy is Context {
    enum CONTRACT_STATE {
        NULL,
        INACTIVE,
        ACTIVE
    ...
    // @TODO change to agreement, proposal or other
    struct Contract {
    ...
    // IAgreemen
    IAgreement private _implementation;
    Transaction[] private _transactions;
    address[2] _parties;

    // address private _party1; // buyer
    // address private _party2; // seller
    ...
    function _excecuteTransaction(uint256 index) private transactionExists(index) transactionExecuted(index) {
        ...
        if (transactionCache.transactionType == TRANSACTION_TYPE.DEFAULT) {
            ...
            bool success = _implementation.agreement(parameterACache, parameterBCache);
            if (success) {
    ...
    function _getTransaction(uint256 index) internal view virtual returns (Transaction memory) {
    ...
    function _updateImplementation(address implement) internal {
        address implementationCache = address(_implementation);
        ...
        _implementation = IAgreement(implement);
        emit ImplementationUpdated(implementationCache, implement);
    }

    /// @notice Retrieves the transaction at the specified index.
    /// @dev This function calls the internal `_getTransaction` function to fetch the `Transaction` object.
    /// @param index The index of the transaction to retrieve.
    /// @return The `Transaction` object corresponding to the given index.
